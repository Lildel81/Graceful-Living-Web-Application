...
const { AdminAvailability, BlockedDate } = require('../models/adminAvailability'); // Availability models
...
const createAppointment = async (req, res) => {
    try {
        ...
        const slotDateTime = createDateTime(new Date(appointmentDate), appointmentTime);
        const now = new Date();
        if (slotDateTime <= now) {
            return res.status(400).json({ 
                success: false, 
        ...
        // STEP 5: Sync with Google Calendar (create event in admin's calendar)
        ...
        // STEP 6: Link Google Calendar event ID to appointment record
        if (eventId) {
...
const getAdminAvailability = async (req, res) => {
    try {
        ...
        const availabilities = await AdminAvailability.find().sort({ dayOfWeek: 1 });
...
const addBlockedDate = async (req, res) => {
    try {
        ...
        console.log('Storing date as:', dateToStore, 'ISO:', dateToStore.toISOString());
...
const getBlockedDates = async (req, res) => {
    try {
        ...
        console.log('Fetched blocked dates from DB:', blockedDates.map(bd => ({ id: bd._id, date: bd.date, dateType: typeof bd.date })));
