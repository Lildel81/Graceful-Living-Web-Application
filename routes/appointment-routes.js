 ...
 */
router.put(
  "/adminportal/appointments/:id/status",
  isAdmin,
...
/**
 * POST /appointments/adminportal/create
 *
 * Creates a new appointment manually from the admin portal.
 * Automatically generates a corresponding Zoom meeting for the
 * specified date and time, and saves both the appointment and
 * meeting details in the database.
 *
 * Request Body:
 * - clientName: string (required) — Full name of the client
 * - clientEmail: string (required) — Email address of the client
 * - clientPhone: string (optional) — Phone number of the client
 * - appointmentDate: string (YYYY-MM-DD, required) — Date of the appointment
 * - appointmentTime: string (HH:mm, required) — Start time of the appointment
 * - notes: string (optional) — Additional notes or comments
 * - _csrf: string (required) — CSRF protection token
 *
 * Response: { success: true, appointment: { ... } }
 */
router.post(
