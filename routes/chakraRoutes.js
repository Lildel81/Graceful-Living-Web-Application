...
function getScore(section, questionIndex, answer) {
  ...
  if (perQuestion && perQuestion.hasOwnProperty(answer)) return perQuestion[answer];
...
router.post("/save", async (req, res) => {
  try {
    ...
    Object.entries(scoredChakras).forEach(([section, questions]) => {
      const total = Object.values(questions).reduce((sum, obj) => sum + obj.score, 0);
    ...
    const experienceValue = req.body.experience === "other" && req.body.experienceOtherText
    ...
    const assessmentResponse = new ChakraAssessment({
      ...
      familiarWith: Array.isArray(req.body.familiarWith) ? req.body.familiarWith : [req.body.familiarWith],
      experienceDetails: req.body.experienceDetails,
      goals: req.body.goals,
      challenges: Array.isArray(req.body.challenges) ? req.body.challenges : [req.body.challenges],
